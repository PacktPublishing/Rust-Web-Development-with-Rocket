...
impl User {
    ...
    pub async fn find_all(
        ...
        if pagination.is_some() {
            return Self::find_all_with_pagination(db, pagination).await;
        } else {
    ...
    async fn find_all_with_pagination(
        ...
        let users = sqlx::query_as::<_, Self>(query_str)
            .bind(pagination.unwrap().next.unwrap())
        ...
        if users.len() == DEFAULT_LIMIT {
            ...
            let connection = db.acquire().await?;
            let exists = sqlx::query_as::<_, TmpBool>(query_str)
                .bind(&users.last().unwrap().created_at)
                .fetch_one(connection)
                .await?;
            if exists.0 {
                new_pagination = Some(Pagination {
                    next: Some(users.last().unwrap().created_at.to_owned()),
                    ..<Pagination as std::default::Default>::default()
                });
            }
        }
...
pub struct NewUser<'r> {
    #[field(validate = len(1..).or_else(msg!("name cannot be empty")))]
    pub username: &'r str,
