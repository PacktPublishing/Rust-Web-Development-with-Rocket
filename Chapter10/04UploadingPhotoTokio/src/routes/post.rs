...
pub async fn create_post<'r>(
    ...
    if mt.is_text() {
        ...
        let _ = tokio::task::spawn_blocking(move || process_photo(&orig_path, &dest_filename))
            .await
            .map_err(|_| create_err())?;
    } else if mt.is_svg() {
        post_type = PostType::Photo;
...
fn process_photo(path: &str, dest_filename: &str) -> Result<(), ()> {
    let orig_file = File::open(path).map_err(|_| ())?;
    ...
    let output_file_path = Path::new(rocket::fs::relative!("static")).join(&dest_filename);
    let mut file_writer = File::create(output_file_path).map_err(|_| ())?;
