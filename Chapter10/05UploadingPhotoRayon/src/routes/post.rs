...
pub async fn create_post<'r>(
    ...
    if mt.is_text() {
        ...
        rayon::spawn(move || {
            let _ = send.send(process_text(&orig_path));
        });
        let text = recv
            .await
            .map_err(|_| create_err())?
        ...
        let orig_path = upload.file.path().unwrap().to_string_lossy().to_string();
        let orig_filename = format!("{}.jpg", file_uuid);
        content.push_str("/assets/");
        content.push_str(&orig_filename);

        let (send, recv) = oneshot::channel();
        rayon::spawn(move || {
            let _ = send.send(process_photo(&orig_path, &orig_filename));
        });
        let _ = recv.await.map_err(|_| create_err())?;
    } else if mt.is_svg() {
        post_type = PostType::Photo;
        let orig_filename = format!("{}.svg", file_uuid);
        content.push_str("/assets/");
        content.push_str(&orig_filename);
        let dest_path = Path::new(rocket::fs::relative!("static")).join(&orig_filename);
        upload
            .file
...
fn process_text(orig_path: &str) -> Result<String, ()> {
    let mut text_content = vec![];
    let _ = File::open(orig_path)
        .map_err(|_| ())?
        .read(&mut text_content)
        .map_err(|_| ())?;
    let text_result = String::from_utf8(text_content).map_err(|_| ())?;
    Ok(text_result)
}

fn process_photo(orig_path: &str, dest_filename: &str) -> Result<(), ()> {
